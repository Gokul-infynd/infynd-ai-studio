    ...
    def _run_migrations(self, should_initialize_alembic, fix) -> None:
        ...
        with buffer_context as buffer:
            ...
            try:
                ...
                # In embedded mode, we may share a DB with other apps (like Infynd)
                # which causes AutogenerateDiffsDetected because of external tables.
                # We log it but don't exit if it's not a 'fix' run.
                logger.warning(f"Self-check: models and database mismatch, ignoring in embedded mode. {exc}")
